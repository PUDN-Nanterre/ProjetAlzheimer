...
-   **M2 Information Communication, Données et Société Université Paris Nanterre**
...
Il existe deux méthodes d'exportation des résultats d'une requête dans HAL en vue de leur traitement dans le logiciel R :

**a) La recherche sur l'interface de HAL :**

1.  Accéder à HAL via "<https://hal.archives-ouvertes.fr/>".

2.  Consulter la documentation pour comprendre la recherche simple et avancée.

3.  Utiliser les opérateurs booléens pour combiner les termes de recherche.

4.  Formuler la requête en respectant les règles d'espacement et de casse des opérateurs.

5.  Exporter les résultats dans des formats tels que XML-TEI, BibTeX, EndNote, CSV, PDF, RTF.

6.  Cocher les dépôts à exporter ou exporter tous les résultats.

7.  Sélectionner le format de fichier souhaité dans l'option "Actions".

8.  Utiliser "Export avancé" pour choisir et réorganiser les champs.

9.  Prévisualiser ou télécharger l'export.

10. Filtrer les résultats si la requête dépasse 1000 résultats pour éviter une exportation partielle.

**b) L'interrogation de l'API de HAL**

Dans cette méthode, tous les paramètres de la requête sont précisés dans l'URL, notamment la valeur recherchée, le format du fichier de sortie, et les champs souhaités dans le fichier de sortie.

La documentation de HAL, disponible à l'adresse "<https://api.archives-ouvertes.fr/docs/search>", explique comment faire une requête via l'API de HAL et comment récupérer les résultats dans un fichier de sortie.

Interroger l'API de HAL en spécifiant tous les paramètres nécessaires dans l'URL offre une manière flexible de récupérer les données selon les critères de recherche spécifiés.
...
Pour obtenir les informations nécessaires, nous utilisons les requêtes suivantes pour interroger l'API HAL. Nous sélectionnons spécifiquement les champs souhaités de l'API dans nos requêtes :
...
L'étiquetage des domaines dans ce contexte consiste à regrouper les domaines spécifiques en catégories plus larges afin de simplifier l'analyse et la compréhension des données. Cela permet de gérer et interpréter plus facilement une diversité de domaines scientifiques dans un ensemble de données. En utilisant des étiquettes, nous simplifions l'arborescence des domaines dans HAL, ce qui réduit la complexité des analyses et des visualisations tout en préservant les informations essentielles sur les disciplines académiques des articles.

Nous allons maintenant attribuer à nos domaines des étiquettes et nous les agrégeons pour créer des catégories plus larges.
...
Les données recodées sont sauvegardées dans le fichier "AlzheimerHAL.Rda" qui sera utilisé pour les analyses.
...
Il convient de souligner que la classification des SHS ne se limite pas à une seule variable "domaine", et que leur présence ne garantit pas nécessairement qu'elles soient le domaine principal de la notice. En effet, explorer l'intersection des SHS avec d'autres domaines revêt une importance particulière pour notre analyse.

C'est pourquoi nous nous appuierons sur la colonne "domain_s", qui renseigne sur le domaine secondaire en plus de la colonne "primaryDomain_s" :
...
Il est crucial d'expliquer nos regroupements :

-   Nous avons regroupé les types "ART" et "ISSUE" ensemble. "ART" désigne un article publié dans une revue, tandis que "ISSUE" fait référence à un numéro spécial de revue. Nous avons pris cette décision afin de rassembler tous les documents relevant du domaine des revues et ainsi constituer par la suite un sous-corpus spécifique dédié à ce type de publication.

-   Les "PROCEEDINGS" sont généralement les actes d'une conférence, incluant les articles, les présentations, et autres contributions. Étant donné que les "PROCEEDINGS" sont une forme courante de communication dans les congrès, il est logique de les regrouper avec les "COMM" (Communications dans un congrès).

-   Les "HDR" (Habilitation à Diriger des Recherches) sont des diplômes universitaires. Ils sont similaires aux thèses dans le sens où ils représentent une étape avancée dans la carrière académique d'un chercheur.

-   Les "COUV" (Chapitre d'ouvrage) sont des parties intégrantes d'un ouvrage plus vaste, souvent publiés individuellement dans des anthologies ou des compilations. Étant donné leur lien étroit avec les ouvrages plus larges, les regrouper avec les "Ouvrages" permet une meilleure organisation.

```{r, include=TRUE}
dataset_shs_alzheimer <- dataset_shs_alzheimer %>%
  mutate(type_publication = case_when(
...
L’API nous fournit toutes les informations des colonnes sélectionnées, cependant, certaines données ne sont pas pertinentes pour notre recherche. Afin de nous concentrer uniquement sur les informations relatives aux revues SHS, nous créons un sous-ensemble de données.

Grâce à notre agrégation précédente, nous avons toutes les données des articles de revues réunies.

```{r, include=TRUE}
unique_doctype <- unique(dataset_shs_alzheimer$type_publication) 
print(unique_doctype)
```

Nous allons d'abord filtrer le jeu de données SHS pour inclure uniquement les lignes où le type de publication est "Article dans une revue" et où la colonne journalTitle_s contient les mots "Revue" ou "revue".

Notons qu'on aurait pu inclure le mot "review" en anglais dans notre recherche, mais cela représente un risque, car le terme "review" peut également désigner des recensions critiques d'ouvrages.

Il est également important de noter que les lignes marquées comme "UNDEFINED" correspondent à des documents où la valeur de la colonne journalTitle_s n'est pas renseignée. Par précaution, nous avons inclus cette condition pour couvrir les cas futurs où les lignes pourraient également contenir le mot "revue" dans la colonne journalTitle_s.

Ce qui serait également intéressant à faire pour ne manquer aucun document de revues qui est marqué comme "UNDEFINED", et dans le cas où la variable journalTitle_s est fournie, serait d'examiner manuellement les titres pour déterminer s'ils correspondent à des revues académiques. Bien que cela puisse être fastidieux, c'est une méthode précise pour identifier les revues.

```{r, include=TRUE}
revues_shs_alzheimer <- dataset_shs_alzheimer %>%
  filter(
    type_publication %in% c("Article dans une revue") |
      str_detect(tolower(journalTitle_s), "revue")
  )
```

Le nombre de publications de revues :

```{r, include=TRUE}
nrow(revues_shs_alzheimer)
```

**LANGUES**

Les publications sont répertoriées dans différentes langues selon leur saisie sur HAL. La langue est généralement déclarée lors du dépôt de la publication. Cependant, de nombreuses erreurs de saisie peuvent exister. Afin d'établir une répartition plus précise des contenus, nous allons tenter de détecter automatiquement la langue, puis vérifier si la langue saisie correspond à celle détectée automatiquement. Dans le cas où les langues ne correspondent pas, nous procéderons à un nettoyage manuel.

Cette opération de nettoyage de la langue sera effectuée uniquement sur notre sous-corpus de revues, car notre analyse linguistique est spécifiquement axée sur les revues.
...
Avec cette librairie et son fonctionnement, nous rencontrons un obstacle pour les publications incluant :
...
On observe également l'absence de correction de la langue saisie dans le sens français vers anglais, ce qui peut s'expliquer par plusieurs facteurs. Il est possible qu'il y ait eu des erreurs de manipulation lors du dépôt des publications sur HAL, notamment des oublis ou des sélections incorrectes de la langue. Il est également envisageable que HAL attribue une langue par défaut en l'absence de saisie, en se basant sur les autres données renseignées.

C'est pour cette raison que nous avons choisi de vérifier manueloctylement pour avoir des données bien propres.
...
Pour ce faire, la transformation est réalisée avec la fonction **`if_else`**, qui permet de remplacer les occurrences de "Elsevier" dans la colonne **`journalPublisher_s`** par "Elsevier Masson".
...
tendances_publication <- dataset_shs_alzheimer %>%
  group_by(publicationDateY_i, type_publication) %>%
  summarise(count = n()) %>%
  filter(!(type_publication %in% c("UNDEFINED")))
...
ggplot(tendances_publication, aes(x = publicationDateY_i, y = count, fill = type_publication)) +
...
*La première publication en 1998 : "Les processus d’inhibition dans le vieillissement: Aspects méthodologiques et diagnostiques" par Denis Brouillet, Arielle Syssau Vaccarella, Isabelle Gimet (un chapitre d'un livre titré "Actualités 1998 sur la maladie d'Alzheimer et les syndromes apparentés").*
...
Avant de créer le camembert, nous allons tout d'abord créer un résumé des comptes et pourcentages pour chaque catégorie type_publication:
...
count_data <- dataset_shs_alzheimer %>%
  count(type_publication) %>%  # Calcul du nombre d'occurrences par type de document
...
ggplot(count_data, aes(x = "", y = n, fill = type_publication)) +
  geom_bar(width = 1, stat = "identity", color = "white") +  

  geom_label_repel(aes(
    label = scales::percent(percentage / 100),
    fill = type_publication
  ),
...
En premier lieu, nous allons distinguer les auteurs de chaque publication.
...
Le mot 'personnes' est le plus couramment utilisé dans les titres des publications françaises. Cependant, étant donné que ce mot est neutre, il est préférable de faire référence aux mots en violet les plus utilisés en deuxième place, tels que 'cognitive' et 'mémoire', etc.

Dans le nuage de mot français, on remarque que le terme “mild” est toujours présent. En effet, les anglicismes dans les titres français n’étaient pas détectés et apparaissent donc dans notre nuage de mots français. Il est pertinent de les garder puisqu’ils sont utilisés dans des titres français.

D’autres mots anglais peuvent apparaître malgré le filtrage de la langue, dû au fait que certains auteurs mettent leur titre en français et en anglais.
...
Du côté de la recherche anglophone, les termes qui ressortent sont relatifs à l’aspect concret de la maladie, ce qui se passe de manière technique et scientifique chez les personnes concernées par cette maladie.
